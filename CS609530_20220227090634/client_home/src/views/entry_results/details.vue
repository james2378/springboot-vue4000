<template>
	<div class="diy_details page_entry_results" id="entry_results_details">
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_entry_results'>
							<div_entry_results :obj="obj"></div_entry_results>
						</div>
					</div>
				</div>
			</div>
...
<script>
	import div_entry_results from "@/components/diy/div_entry_results.vue";
	...
	export default {
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/entry_results/set?entry_results_id=' + obj["entry_results_id"], {
					hits
				}, (res) => {
					if(res.result){
						...
						var body = {
							...
							source_field: "entry_results_id",
							source_id: this.obj.entry_results_id,
			...
			get_collect(obj) {
				...
				var query = {
					source_table: "entry_results",
					source_field: "entry_results_id",
					source_id: obj["entry_results_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/entry_results/set?entry_results_id=' + _this.obj["entry_results_id"], {
								praise_len
							}, (res) => {
								if(res.result){
