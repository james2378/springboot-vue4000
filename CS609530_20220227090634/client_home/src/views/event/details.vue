<template>
	<div class="diy_details page_event" id="event_details">
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_event'>
							<div_event :obj="obj"></div_event>
...
<script>
	import div_event from "@/components/diy/div_event.vue";
	...
	export default {
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/event/set?event_id=' + obj["event_id"], {
					hits
				}, (res) => {
					if(res.result){
			...
			get_obj_after(json) {
				...
				if (json && json.result && json.result.obj) {
					...
					this.form_comment.source_id = this.obj.event_id;
					this.runTiming();
				}
			},
			/**
