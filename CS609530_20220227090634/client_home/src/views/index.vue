		        ...
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/event/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_event = !show_list_event">
		                        <div class="switch_box">
		                            <span v-if="show_list_event"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!--比赛项目推荐列表组件 -->
		                <list_event v-if="show_list_event" :list="list_event" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_event.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_event" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_event" :key="i" @click="to_details('event',o,'event_id')">
		                            <td v-for="(oj,n) in fields_event">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
		...
		data() {
			return {
				...
				fields_event: [
					...
					{
						key: "competition_cover",
						label: "比赛封面",
						type: "图片"
					},
					{
						key: "competition_introduction",
						label: "比赛简介",
						type: "多文本"
					},
				],
		...
		methods: {
			...
			get_event() {
				...
				this.$get(url, {
					...
					if (json.result) {
						this.list_event = json.result.list;
					}
				})
			},
