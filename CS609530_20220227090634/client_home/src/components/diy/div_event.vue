<template>
	<div class="diy_details_box diy_div_event">
		<div class="warp">
			<div class="container">
				...
				<div class="row">
						...
						<!-- 图片 -->
							...
							<div class="details_btn_wrap">
								<button class="btn btn-primary details_btn" v-if="$check_action('/entry_information/edit','add')" @click="to_form('/entry_information/edit')" >报名 </button>
								<button class="btn btn-primary details_btn" v-if="$check_action('/event/details','set')" @click="change_collect()" >收藏</button>
							</div>
						<!-- 富文本 -->
		...
		methods: {
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/event/set?event_id=' + _this.obj["event_id"], {
								praise_len
							}, (res) => {
								if(res.result){
			...
			change_collect() {
				...
				if (this.state_collect) {
					...
					query.img = this.obj.competition_cover
