<template>
	<div class="diy_details_box diy_div_score_ranking">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "score_ranking_id",
					source_id: this.obj["score_ranking_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/score_ranking/set?score_ranking_id=' + _this.obj["score_ranking_id"], {
								praise_len
							}, (res) => {
								if(res.result){
