<template>
	<div class="diy_details_box diy_div_referee">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "referee_id",
					source_id: this.obj["referee_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/referee/set?referee_id=' + _this.obj["referee_id"], {
								praise_len
							}, (res) => {
								if(res.result){
