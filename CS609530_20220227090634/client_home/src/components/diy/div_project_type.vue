<template>
	<div class="diy_details_box diy_div_project_type">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "project_type_id",
					source_id: this.obj["project_type_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/project_type/set?project_type_id=' + _this.obj["project_type_id"], {
								praise_len
							}, (res) => {
								if(res.result){
