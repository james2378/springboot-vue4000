<template>
	<div class="diy_details_box diy_div_entry_information">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "entry_information_id",
					source_id: this.obj["entry_information_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/entry_information/set?entry_information_id=' + _this.obj["entry_information_id"], {
								praise_len
							}, (res) => {
								if(res.result){
