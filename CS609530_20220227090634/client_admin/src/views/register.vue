			...
			<el-row>
				<el-col>
					<el-form :model="form" ref="form" label-width="80px" class="form">
						...
						<!-- 裁判 -->
						<form_referee v-if="form.user_group === '裁判'" :form="form_sub" ></form_referee>
...
<script>
	...
	/* 裁判 */
	import form_referee from "@/components/diy/form_referee";
			...
			async submit_check(param){
				...
				if(!ret && user_group){
					if(user_group === "裁判"){
						...
						if(!ret && !form_sub["referee_number"]){
							ret = "裁判编号不能为空!";
						}
						else if(!ret){
							res = await this.$get("~/api/referee/count?", {"referee_number": form_sub["referee_number"]});
							if(res.result){
								ret = "裁判编号已存在!";
							}
						}
