<template>
	<el-main class="bg">
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','full_name') || $check_field('add','full_name') || $check_field('set','full_name')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="姓名" prop="full_name">
					<el-input id="full_name" v-model="form_sub['full_name']" placeholder="请输入姓名"
							  v-if="user_group === '管理员' || (form_sub['referee_id'] && $check_field('set','full_name')) || (!form_sub['referee_id'] && $check_field('add','full_name'))"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','gender') || $check_field('add','gender') || $check_field('set','gender')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="性别" prop="gender">
					<el-select id="gender" v-model="form_sub['gender']"
						v-if="user_group === '管理员' || (form_sub['referee_id'] && $check_field('set','gender')) || (!form_sub['referee_id'] && $check_field('add','gender'))">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','referee_number') || $check_field('add','referee_number') || $check_field('set','referee_number')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="裁判编号" prop="referee_number">
					<el-input id="referee_number" v-model="form_sub['referee_number']" placeholder="请输入裁判编号"
							  v-if="user_group === '管理员' || (form_sub['referee_id'] && $check_field('set','referee_number')) || (!form_sub['referee_id'] && $check_field('add','referee_number'))"></el-input>
					<div v-else-if="$check_field('get','referee_number')">{{form_sub['referee_number']}}</div>
				</el-form-item>
			</el-col>
			<el-col :xs="24" :sm="12" :lg="8">
				<el-form-item>
...
<script>
	...
	export default {
		...
		data() {
			return {
				...
				disabledObj:{
					...
					"referee_number_isDisabled": false,
				},
		...
		methods: {
			...
			// 提交附加信息
			submit_sub(form_sub){
				if(form_sub["referee_id"]){
					// 提交事件
					this.$post("~/api/" + this.group_table + "/set?referee_id=" + form_sub["referee_id"],form_sub,(res)=>{
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/referee/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/referee/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/referee/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/referee/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/referee/view','get');
