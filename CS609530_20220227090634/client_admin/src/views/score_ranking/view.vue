		...
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			<el-col v-if="user_group === '管理员' || $check_field('get','entry_name') || $check_field('add','entry_name') || $check_field('set','entry_name')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="项目名称" prop="entry_name">
					<el-input id="entry_name" v-model="form['entry_name']" placeholder="请输入项目名称"
							  v-if="user_group === '管理员' || (form['score_ranking_id'] && $check_field('set','entry_name')) || (!form['score_ranking_id'] && $check_field('add','entry_name'))" :disabled="disabledObj['entry_name_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','competition_place') || $check_field('add','competition_place') || $check_field('set','competition_place')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="比赛地点" prop="competition_place">
					<el-input id="competition_place" v-model="form['competition_place']" placeholder="请输入比赛地点"
							  v-if="user_group === '管理员' || (form['score_ranking_id'] && $check_field('set','competition_place')) || (!form['score_ranking_id'] && $check_field('add','competition_place'))" :disabled="disabledObj['competition_place_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','game_time') || $check_field('add','game_time') || $check_field('set','game_time')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="比赛时间" prop="game_time">
					<el-date-picker :disabled="disabledObj['game_time_isDisabled']" v-if="user_group === '管理员' || (form['score_ranking_id'] && $check_field('set','game_time')) || (!form['score_ranking_id'] && $check_field('add','game_time'))" id="game_time"
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','student_number') || $check_field('add','student_number') || $check_field('set','student_number')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="学生编号" prop="student_number">
						<el-select v-if="user_group === '管理员' || (form['score_ranking_id'] && $check_field('set','student_number')) || (!form['score_ranking_id'] && $check_field('add','student_number'))" id="student_number" v-model="form['student_number']" :disabled="disabledObj['student_number_isDisabled']">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','student_name') || $check_field('add','student_name') || $check_field('set','student_name')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="学生姓名" prop="student_name">
					<el-input id="student_name" v-model="form['student_name']" placeholder="请输入学生姓名"
							  v-if="user_group === '管理员' || (form['score_ranking_id'] && $check_field('set','student_name')) || (!form['score_ranking_id'] && $check_field('add','student_name'))" :disabled="disabledObj['student_name_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','score') || $check_field('add','score') || $check_field('set','score')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="比赛成绩" prop="score">
					<el-input id="score" v-model="form['score']" placeholder="请输入比赛成绩"
							  v-if="user_group === '管理员' || (form['score_ranking_id'] && $check_field('set','score')) || (!form['score_ranking_id'] && $check_field('add','score'))" :disabled="disabledObj['score_isDisabled']"></el-input>
					<div v-else-if="$check_field('get','score')">{{form['score']}}</div>
				</el-form-item>
			</el-col>
			<el-col v-if="user_group === '管理员' || $check_field('get','score_ranking') || $check_field('add','score_ranking') || $check_field('set','score_ranking')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="成绩排名" prop="score_ranking">
					<el-input id="score_ranking" v-model="form['score_ranking']" placeholder="请输入成绩排名"
							  v-if="user_group === '管理员' || (form['score_ranking_id'] && $check_field('set','score_ranking')) || (!form['score_ranking_id'] && $check_field('add','score_ranking'))" :disabled="disabledObj['score_ranking_isDisabled']"></el-input>
					<div v-else-if="$check_field('get','score_ranking')">{{form['score_ranking']}}</div>
				</el-form-item>
			</el-col>
...
<script>
	...
	export default {
		...
		data() {
			return {
				field: "score_ranking_id",
				url_add: "~/api/score_ranking/add?",
				url_set: "~/api/score_ranking/set?",
				url_get_obj: "~/api/score_ranking/get_obj?",
				url_upload: "~/api/score_ranking/upload?",
				...
				disabledObj:{
					...
					"score_ranking_isDisabled": false,
				},
				// 用户列表
		...
		methods: {
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/score_ranking/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/score_ranking/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/score_ranking/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/score_ranking/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/score_ranking/view','get');
